  ...
  return (
    <div className="card-vibrant anim-fade-up" style={{ padding: 16, marginTop: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <span style={{ fontSize: 13, fontWeight: 800, color: '#C29B4A' }}>📅 أخبار اليوم عالية التأثير</span>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {events.map((e, i) => (
          <div key={`${e.title}-${i}`} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span className="ltr-num" style={{ fontSize: 13, fontWeight: 800, color: '#BB5B5B', minWidth: 44 }}>
              {formatJerusalemTime(e.date)}
            </span>
            <span style={{ fontSize: 9, fontWeight: 800, color: '#BB5B5B', padding: '2px 7px', border: '1px solid rgba(187,91,91,0.3)', borderRadius: 6 }}>
              {e.currency}
            </span>
